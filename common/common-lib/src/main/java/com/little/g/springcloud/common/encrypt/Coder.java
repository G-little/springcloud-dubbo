package com.little.g.springcloud.common.encrypt;
...
/**
 * 各种加密算法 包括：URLEncode、MD5、SHA、HMAC
 */
public class Coder {
	...
	public static String encryptBase64URLSafeString(String key) throws Exception {
		return Base64.encodeBase64URLSafeString(
				key.getBytes(GlobalConstants.DEFAULT_CONTENT_CHARSET));
	}
	...
	public static byte[] encryptMD5_Byte(String data) throws Exception {
		...
		MessageDigest md5 = MessageDigest.getInstance(KEY_MD5);
		md5.update(data.getBytes(GlobalConstants.DEFAULT_CONTENT_CHARSET));
		return md5.digest();
	}
	...
	public static byte[] encryptHMAC(String data, byte[] key) throws Exception {
		...
		return mac.doFinal(data.getBytes(GlobalConstants.DEFAULT_CONTENT_CHARSET));
	...
	public static String generatorCode(String mobile, String deviceId, Long ct) {
		...
		if (!Strings.isNullOrEmpty(mobile) && !Strings.isNullOrEmpty(deviceId)
			...
			try {
				...
				logger.error("calculate default code error:mobile:{},deviceId:{},ct:{}",
						mobile, deviceId, ct, e);
			}
		}
		return code;
	}
