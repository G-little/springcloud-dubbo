...
public class ReloadableResourceBundleMessageSource extends AbstractMessageSource
	...
	private static final Logger logger = LoggerFactory
			.getLogger(ReloadableResourceBundleMessageSource.class);
	 ...
	 * {@link org.springframework.context.support.ResourceBundleMessageSource} referring
	 * to a Spring resource location: e.g. "WEB-INF/messages" for
	 * "WEB-INF/messages.properties", "WEB-INF/messages_en.properties", etc.
	 * <p>
	 * XML properties files are also supported: .g. "WEB-INF/messages" will find and load
	 ...
	 * <p>
	 * Only applies to classic properties files, not to XML files.
	 * @param fileEncodings Properties with filenames as keys and charset names as values.
	 * Filenames have to match the basename syntax, with optional locale-specific
	 * appendices: e.g. "WEB-INF/messages" or "WEB-INF/messages_en".
	 * @see #setBasenames
	 * @see PropertiesPersister#load
	 */
	 ...
	 * <code>java.util.ResourceBundle</code>. However, this is often not desirable in an
	...
	protected String resolveCodeWithoutArguments(String code, Locale locale) {
		if (this.cacheMillis < 0) {
			...
			if (propHolder != null) {
				logger.info("bundle expire,reload bundle messages,"
	...
	protected List<String> calculateAllFilenames(String basename, Locale locale) {
		synchronized (this.cachedFilenames) {
			...
			if (this.fallbackToSystemLocale && !locale.equals(Locale.getDefault())) {
				List<String> fallbackFilenames = calculateFilenamesForLocale(basename,
						Locale.getDefault());
	...
	protected PropertiesHolder getProperties(String filename) {
		synchronized (this.cachedProperties) {
			...
			if (propHolder != null && (propHolder.getRefreshTimestamp() < 0
					|| propHolder.getRefreshTimestamp() > System.currentTimeMillis()
							- this.cacheMillis)) {
				// up to date
				return propHolder;
			}
	 ...
	 * <code>null</code> if not cached before, or a timed-out cache entry (potentially
	...
	protected PropertiesHolder refreshProperties(String filename,
		...
		try {
			if (this.resourceLoader instanceof ResourcePatternResolver) {
				logger.debug("resolve property files by pattern:{},resourceLoader:{}",
						filename + PROPERTIES_SUFFIX, resourceLoader);
				resources = ((ResourcePatternResolver) this.resourceLoader)
						.getResources(filename + PROPERTIES_SUFFIX);
				if (resources == null || resources.length == 0) {
					resources = ((ResourcePatternResolver) this.resourceLoader)
							.getResources(filename + XML_SUFFIX);
				}
			}
			else {
				Resource resource = this.resourceLoader
						.getResource(filename + PROPERTIES_SUFFIX);
				if (!resource.exists()) {
				...
				for (Resource resource : resources) {
					if (resource.exists()) {
						...
						if (this.cacheMillis >= 0) {
							...
							/*
							 * if (propHolder.getFileTimestamp() == fileTimestamp) { if
							 * (logger.isDebugEnabled()) {
							 * logger.debug("Re-caching properties for filename [" +
							 * filename + "] - file hasn't been modified"); }
							 * propHolder.setRefreshTimestamp(refreshTimestamp); return
							 * propHolder; }
							 */
						}
						...
						try {
							...
							if (logger.isWarnEnabled()) {
								logger.warn("Could not parse properties file ["
						...
						if (logger.isDebugEnabled()) {
							logger.debug("No properties file found for ["
