...
import com.little.g.springcloud.common.constants.GlobalConstants;
import com.little.g.springcloud.common.dto.TokenInfo;
import com.little.g.springcloud.common.enums.TokenErrorType;
...
import com.little.g.springcloud.user.TokenCommonUtil;
import com.little.g.springcloud.user.TokenVersionConfig;
import com.little.g.springcloud.user.TokenVersionFactory;
...
public class TokenVerifyInterceptor extends HandlerInterceptorAdapter {
	...
	/**
	 * 检测token是否正确 1.校验版本 2.自校验是否正确 3.是否过有效期
	 * @param token token
	 * @param deviceId deviceId
	 * @return token的有效性对象
	 */
	public static TokenInfo checkToken(String token, String deviceId)
		...
		try {
			if (!checkTokenSelf(token)) {
				log.warn("checkToken,token self error,token:{},deviceId:{}", token,
						deviceId);
				ti.setOK(false);
			...
			log.error("checkToken IOException exception.token:{},deviceId:{},error:{}",
					token, deviceId, e);
			return null;
		}
		// token是否过有效期
		if (!checkTokenExpDate(token)) {
			...
			log.warn("checkToken,token expired error,token:{},deviceId:{}", token,
					deviceId);
			return ti;
		}
	...
	public static boolean checkTokenExpDate(String token) {
		...
		return createDateTime + GlobalConstants.ACCESS_TOKEN_EXPIRSED > nowTime;
	}
	...
	private static boolean checkTokenSelf(String token) throws IOException {
		...
		byte[] sidMd5Bytes = TokenCommonUtil.byteMerger(sidCheckBytes,
				check_token_key.getBytes());
		sidMd5Bytes = TokenCommonUtil.calculateMD5(sidMd5Bytes);
		// 根据配置读MD5后的相应元素组成自校验位
		byte[] calculateCheckByte = new byte[] { sidMd5Bytes[i1], sidMd5Bytes[i2],
