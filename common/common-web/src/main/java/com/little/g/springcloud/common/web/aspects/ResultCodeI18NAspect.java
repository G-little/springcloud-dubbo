package com.little.g.springcloud.common.web.aspects;
...
/**
 * support code message not exist. Created by lengligang on 15/11/26.
 */
@Aspect
public class ResultCodeI18NAspect {

	private static final Logger logger = LoggerFactory
			.getLogger(ResultCodeI18NAspect.class);

	/*
	 * modify by wangzhen 2016-02-01 15:15:00 change @Resource to @Autowired,declare
	 * exclusive one message source
	 */
	@Autowired
	...
	public void modifyReturnMsg(ResultJson result) {
		...
		if (!StringUtils.isEmpty(result.getM())) {
			if (result.getM().startsWith("msg.")) {
				result.setM(getI18NResultMsg(result.getM(),
	...
	private String getI18NResultMsg(String code, Locale locale) {
		try {
			...
			logger.warn("result json does not include the code({}): message,plz fix",
					code);
			return "";
		}
	}
