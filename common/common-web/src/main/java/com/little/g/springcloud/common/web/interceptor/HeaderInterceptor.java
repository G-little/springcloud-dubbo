...
public class HeaderInterceptor extends HandlerInterceptorAdapter {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response,
		...
		Integer isZip = StringUtils.isEmpty(isZipHeader) ? null
				: Integer.valueOf(isZipHeader);
		...
		if (cookies != null && cookies.length != 0) {
			...
			for (int i = 0; i < size; i++) {
				...
				if (cookie.getName().equals("os") && Strings.isNullOrEmpty(osHeader)) {
				...
				else if (cookie.getName().equals("appVersion")
						&& Strings.isNullOrEmpty(appVersion)) {
					appVersion = cookie.getValue();
				}
				else if (cookie.getName().equals("isZip")
						&& Strings.isNullOrEmpty(isZipHeader)) {
					...
					ipAddress = cookie.getValue();
						...
						&& Strings.isNullOrEmpty(deviceIdHeader)) {
					deviceIdHeader = cookie.getValue();
				}
				else if (cookie.getName().equals("deviceType")
