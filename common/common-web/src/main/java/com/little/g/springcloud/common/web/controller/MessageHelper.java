package com.little.g.springcloud.common.web.controller;
...
public class MessageHelper {
	...
	private static final String KINSTALK_ROOT_PACKAGE = "com.little.g";

	private static final Map<Integer, ResultJson> RESULT_JSON_MAP = Maps
			.newConcurrentMap();
	...
	public void init() {
		...
		logger.info("<<<<<<<=========================>>>>>>>>>");
		logger.info("init messageSource:{}, id:{}", messageSource,
				System.identityHashCode(messageSource));
	...
	public synchronized ResultJson getCachedResultJson(Integer code, String msg,
			Locale locale) {
		...
		if (resultJson == null && codes.contains(code)) {
			...
			if (msg == null || Strings.isNullOrEmpty(msg.trim())) {
				String msgLocal = getMessage(code, locale);
				logger.info("messageSource get code:{} msg:{} locale:{} ", code, msgLocal,
						locale);
				resultJson.setM(msgLocal);
			}
			else {
				logger.info("set custom result code:{} msg:{} locale:{}", code, msg,
						locale);
				resultJson.setM(msg);
			}

			logger.info("load one error code ,code:{} old msg:{} msg:{}", code, msg,
					resultJson.getM());
