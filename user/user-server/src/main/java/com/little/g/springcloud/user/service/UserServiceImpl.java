...
import com.little.g.springcloud.common.enums.UserStatus;
...
import com.little.g.springcloud.user.mapper.UserMapper;
import com.little.g.springcloud.user.mapper.UserMapperExt;
...
public class UserServiceImpl implements UserService {
	...
	@Transactional
	@Override
	public ResultJson joinin(@NotEmpty String mobile,
			@NotBlank @Size(min = 4, max = 6) String smscode, @NotBlank String deviceId,
			Byte deviceType, String os) {
		// 校验短信验证码是否正常
		ResultJson r = new ResultJson();
		boolean smsResult = smsService.checkSms(mobile, deviceId, smscode,
				SmsInterType.REG_OR_LOGIN.getValue());
		if (!smsResult) {
	...
	@Override
	public UserJoininDTO createLoginReturn(@NotBlank String deviceId, Byte deviceType,
			String os, UserDTO user) {
		UserDeviceTokenDTO token = tokenService.createToken(user.getUid(), deviceId,
				deviceType, os);
