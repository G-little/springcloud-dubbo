...
import com.little.g.springcloud.pay.dto.FrozenAccount;
...
import com.little.g.springcloud.pay.mapper.FrozenRecordMapperExt;
import com.little.g.springcloud.pay.mapper.TransactionRecordMapper;
import com.little.g.springcloud.pay.mapper.TransactionRecordMapperExt;
...
public class FrozenAccountAdaptor implements AccountAdaptor<FrozenAccount> {
	...
	public void lock(FrozenAccount account) {
		...
		if (frozenRecordMapper.updateByExampleSelective(record, example) <= 0) {
			throw new ServiceDataException(PayErrorCodes.PAY_ERROR,
					"msg.account.frozen.transaction.not.exist");
		}
	}
	...
	public long getBalance(FrozenAccount account) {
		...
		List<TransactionRecord> details = transactionRecordMapper
		...
		unfrozenExample.or().andAccountIdEqualTo(account.getId())
				.andTypeEqualTo(TransactionType.OUTCOME.getValue());

		Long totalUnfrozen = transactionRecordMapperExt
				.sumMoneyByExample(unfrozenExample);
	...
	public AccountDetailDTO getDetail(FrozenAccount account) {
		...
		List<FrozenRecord> frozenRecords = frozenRecordMapper
	...
	public void credit(FrozenAccount account, long amount, Account from) {
		...
		try {
			...
			throw new ServiceDataException(PayErrorCodes.PAY_ERROR,
					"msg.account.duplicate.transaction");
		}
	...
	public void debit(FrozenAccount account, long amount, Account to) {
		...
		if (frozenRecordMapperExt.unfroze(account.getTransNum(), amount) <= 0) {
			log.error("transfer money exception params[account:{},amount:{},to:{}]",
					account, amount, to);
			throw new ServiceDataException(PayErrorCodes.PAY_ERROR,
					"msg.account.frozen.money.not.enough");
		}
	}
