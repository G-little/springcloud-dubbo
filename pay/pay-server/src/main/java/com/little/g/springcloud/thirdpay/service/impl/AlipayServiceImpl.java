...
public class AlipayServiceImpl extends ThirdPayService {
	...
	public PrePayResult prepay(PrepayParams prepayParams) {
		...
		model.setTotalAmount(
				String.valueOf(MoneyUtil.long2Double(prepayParams.getMoney())));
		...
		try {
			...
			return new PrePayResult(response.getOutTradeNo(), response.getBody(),
					response.getTradeNo());
		}
	...
	public PayCallbackInfo queryPay(QueryPayParams params) {
		...
		try {
			...
			callback.setRealFee(
					MoneyUtil.double2Long(new Double(response.getTotalAmount())));
	...
	private ThirdPayStatus genThirdStatus(String status) {
		...
		else if ("TRADE_SUCCESS".equals(status) || "TRADE_FINISHED".equals(status)) { // 无法处理的状态
	...
	public PayCallbackInfo verifyResponse(Map<String, String> params) {
		...
		try {
			...
			if (!AlipaySignature.rsaCheckV1(params, config.getPublickey(), "UTF-8",
					"RSA2")) {
				throw new PayException(PayErrorCodes.THIRDPAY_ERROR,
						"msg.thirdpay.notify.invalid");
			}
			...
			callbackInfo.setRealFee(
					MoneyUtil.double2Long(new Double(params.get("total_amount"))));
			if (StringUtils.isNotEmpty(params.get("gmt_payment"))) {
				try {
					callbackInfo.setPayTime(DateUtils
							.parseDate(params.get("gmt_payment"), "yyyy-MM-dd HH:mm:ss")
							.getTime());
					callbackInfo.setFinishTime(callbackInfo.getPayTime());
				}
				catch (ParseException e) {
					logger.error("parse paytime error.paytime:{}",
							params.get("gmt_payment"), e);
				}
			}
	...
	public PreRefundResult refund(RefundParams params) {
		...
		model.setRefundAmount(
				String.valueOf(MoneyUtil.long2Double(params.getRefundFee())));
