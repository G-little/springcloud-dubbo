...
public class LitemallCommentServiceImpl implements LitemallCommentService {
    ...
    public List<LitemallCommentDTO> queryGoodsByGid(Integer id, int offset, int limit) {
        ...
        return DTOUtil.convert2List(commentMapper.selectByExample(example), LitemallCommentDTO.class);
    }

    @Override
    public List<LitemallCommentDTO> query(Byte type, Integer valueId, Integer showType, Integer offset, Integer limit) {
        LitemallCommentExample example = new LitemallCommentExample();
        example.setOrderByClause(LitemallCommentDTO.Column.addTime.desc());
        if (showType == 0) {
    ...
    public int save(LitemallCommentDTO comment) {
        ...
        return commentMapper.insertSelective(DTOUtil.convert2T(comment, LitemallComment.class));
    }

    @Override
    public List<LitemallCommentDTO> querySelective(String userId, String valueId, Integer page, Integer size, String sort, String order) {
        LitemallCommentExample example = new LitemallCommentExample();
    ...
    public LitemallCommentDTO findById(Integer id) {
        return DTOUtil.convert2T(commentMapper.selectByPrimaryKey(id), LitemallCommentDTO.class);
    }

    @Override
    public int updateById(LitemallCommentDTO comment) {
        return commentMapper.updateByPrimaryKeySelective(DTOUtil.convert2T(comment, LitemallComment.class));
    }
}
