...
public class LitemallOrderServiceImpl implements LitemallOrderService {
	...
	public int add(LitemallOrderDTO order) {
		...
		LitemallOrder litemallOrder = DTOUtil.convert2T(order, LitemallOrder.class);
		int r = litemallOrderMapper.insertSelective(litemallOrder);
		order.setId(litemallOrder.getId());
		return r;
	}
	...
	@Override
	public UserOrderInfoDTO orderInfo(Integer userId) {
		LitemallOrderExample example = new LitemallOrderExample();
		example.or().andUserIdEqualTo(userId).andDeletedEqualTo(false);
		...
		UserOrderInfoDTO orderInfo = new UserOrderInfoDTO();
