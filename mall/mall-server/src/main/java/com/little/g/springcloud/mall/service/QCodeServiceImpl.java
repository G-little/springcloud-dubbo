...
public class QCodeServiceImpl implements QCodeService {
    ...
    @Override
    public String createGrouponShareImage(String goodName, String goodPicUrl,
                                          LitemallGrouponDTO groupon) {
        try {
            // 创建该商品的二维码
            File file = wxMaService.getQrcodeService().createWxaCodeUnlimit(
                    "groupon," + groupon.getId(), "pages" + "/index/index");
            ...
            // 存储分享图
            LitemallStorageDTO storageInfo = storageService.store(inputStream2,
                    imageData.length, "image/jpeg",
    ...
    public String createGoodShareImage(String goodId, String goodPicUrl,
        ...
        try {
            ...
            ByteArrayInputStream inputStream2 = new ByteArrayInputStream(imageData);
            // 存储分享图
            LitemallStorageDTO litemallStorage = storageService.store(inputStream2,
                    imageData.length, "image/jpeg", getKeyName(goodId));
    ...
    private byte[] drawPicture(InputStream qrCodeImg, String goodPicUrl, String goodName)
        ...
        // 底层空白 bufferedImage
        BufferedImage baseImage = new BufferedImage(red.getWidth(), red.getHeight(),
        ...
        // 转jpg
        BufferedImage result = new BufferedImage(baseImage.getWidth(),
                baseImage.getHeight(), BufferedImage.TYPE_3BYTE_BGR);
    ...
    private void drawTextInImg(BufferedImage baseImage, String textToWrite, int x,
                               int y) {
        Graphics2D g2D = (Graphics2D) baseImage.getGraphics();
        g2D.setColor(new Color(167, 136, 69));
    ...
    private void drawImgInImg(BufferedImage baseImage, BufferedImage imageToWrite, int x,
