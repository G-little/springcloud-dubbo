...
public class MallSwagger2Configuration {
    ...
    public Docket adminDocket() {
        ...
        tokenPar.name("token").description("认证token")
                .modelRef(new ModelRef("string")).parameterType("header")
                .required(true).build();
        pars.add(tokenPar.build());
        ParameterBuilder devicePar = new ParameterBuilder();
        devicePar.name("deviceId").description("设备号")
                .modelRef(new ModelRef("string")).parameterType("header")
                .required(true).build();
        ...
        return new Docket(DocumentationType.SWAGGER_2)
                ...
                .select()
                .apis(RequestHandlerSelectors.basePackage("com.little.g.springcloud.mall.web"))
