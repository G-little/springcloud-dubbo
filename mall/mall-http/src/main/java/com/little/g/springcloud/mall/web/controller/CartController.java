...
public class CartController {
     ...
     * @return 立即购买操作结果
     */
    @ApiOperation(value = "立即购买", notes = "与加入商品到购物车区别  1. 如果购物车内已经存在购物车货品，前者的逻辑是数量添加，这里的逻辑是数量覆盖\n" +
            "2. 添加成功以后，前者的逻辑是返回当前购物车商品数量，这里的逻辑是返回对应购物车项的ID ")
    @PostMapping("fastadd")
    public ResultJson<Integer> fastadd(@LoginUser Integer userId, @RequestBody LitemallCartDTO cart) {
        if (userId == null) {
            return ResponseUtil.unlogin();
        }
        if (cart == null) {
     ...
     * @return 修改结果
     */
    @ApiOperation(value = "修改购物车商品货品数量", notes = "购物车商品信息 { id: xxx, goodsId: xxx, productId: xxx, number: xxx }")
    @PostMapping("update")
    public ResultJson update(@LoginUser Integer userId, @RequestBody LitemallCartDTO cart) {
        if (userId == null) {
            return ResponseUtil.unlogin();
        }
        Integer productId = cart.getProductId();
    ...
    @PostMapping("checked")
    public ResultJson<CartIndexVo> checked(@LoginUser Integer userId, @RequestBody String body) {
        if (userId == null) {
            return ResponseUtil.unlogin();
        }
        if (body == null) {
    ...
    @PostMapping("delete")
    public ResultJson<CartIndexVo> delete(@LoginUser Integer userId, @RequestBody String body) {
        if (userId == null) {
            return ResponseUtil.unlogin();
        }
        if (body == null) {
