...
public class CartController {
	...
	@ApiImplicitParams({ @ApiImplicitParam(name = "cartId",
			value = "购物车商品ID： 如果购物车商品ID是空，则下单当前用户所有购物车商品； 如果购物车商品ID非空，则只下单当前购物车商品。"),
	...
	public Object checkout(@LoginUser Integer userId,
			...
			@RequestParam(required = false) Integer grouponRulesId) {
		if (userId == null) {
			return ResponseUtil.unlogin();
		}
		...
		if (grouponRulesId != null) {
			grouponRules = grouponRulesService.findById(grouponRulesId);
