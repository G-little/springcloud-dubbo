...
public class CommentController {
    ...
    @ApiOperation("评论数量")
    @ApiImplicitParams({
            @ApiImplicitParam(value = "类型ID。 如果是0，则查询商品评论；如果是1，则查询专题评论。", allowableValues = "0,1"),
            @ApiImplicitParam(value = "商品或专题ID。如果type是0，则是商品ID；如果type是1，则是专题ID。", allowableValues = "0,1")
    })
    @GetMapping("count")
    public ResultJson<CommentCountVo> count(@NotNull Byte type, @NotNull Integer valueId) {
    ...
    @ApiImplicitParams({
            @ApiImplicitParam(name = "type",
                    value = "类型ID。 如果是0，则查询商品评论；如果是1，则查询专题评论。",
                    allowableValues = "0,1", dataType = "Byte", required = true),
            @ApiImplicitParam(name = "valueId",
                    value = "商品或专题ID。如果type是0，则是商品ID；如果type是1，则是专题ID。", dataType = "Integer", required = true),
            @ApiImplicitParam(name = "showType",
                    value = "显示类型。如果是0，则查询全部；如果是1，则查询有图片的评论。", allowableValues = "0,1", dataType = "Integer", required = false),
    ...
    @GetMapping("list")
    public ResultJson<Page<CommentListVo>> list(@NotNull Byte type, @NotNull Integer valueId,
                                                @NotNull Integer showType, @RequestParam(defaultValue = "1") Integer page,
