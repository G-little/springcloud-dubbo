...
public class AftersaleController {
    ...
    @ApiImplicitParams({@ApiImplicitParam(name = "status",
            ...
            allowableValues = "0,1,2,3,4,5", dataType = "Short", required = true),
    ...
    @GetMapping("detail")
    public ResultJson<AftersaleDetailVo> detail(@LoginUser Integer userId, @NotNull Integer orderId) {
        if (userId == null) {
            return ResponseUtil.unlogin();
        }
    ...
    @ApiImplicitParams({
            @ApiImplicitParam(name = "id", value = "售后单ID", dataType = "Integer", required = true)
    })
    @PostMapping("cancel")
